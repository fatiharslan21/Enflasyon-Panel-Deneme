...
def dashboard_modu():
    ...
    with st.sidebar:
        ...
        # CANLI TAKİP MODU (YENİ EKLENDİ)
        st.divider()
        auto_refresh = st.checkbox("🟢 Canlı Takip Modu", value=False,
                                   help="Aktif edilirse sayfa her 10 saniyede bir yenilenir.")
        ...
        for u in users_db.keys():
            ...
            is_online = (datetime.now() - last_seen).total_seconds() < 300  # 5 dk
            ...
            if not pivot.empty:
                ...
                with t4:  # ASİSTAN (AKILLI)
                    ...
                    if submitted and q:
                        ...
                        if not res.empty:
                            if len(res) > 1:
                                st.info("Birden fazla sonuç bulundu, en ilgili olanı gösteriyorum:")
                                # Selectbox'ı formun dışına koyuyoruz ki seçim yapabilsin
                                secim = st.selectbox("Seçiniz:", res[ad_col].unique())
                                t = df_analiz[df_analiz[ad_col] == secim].iloc[0]
                            else:
                                t = res.iloc[0]
