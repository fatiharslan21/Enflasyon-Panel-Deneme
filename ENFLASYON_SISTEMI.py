...
# --- 1. AYARLAR ---
# Sidebar (Yan Panel) varsayılan olarak AÇIK (expanded) başlasın
...
# --- CSS (SADECE GÖRSEL GÜZELLEŞTİRME - GİZLEME YOK) ---
st.markdown("""
    <style>
        @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;600;800&family=JetBrains+Mono:wght@400&display=swap');
        .stApp { background-color: #f8fafc; font-family: 'Inter', sans-serif; color: #1e293b; }

        /* SADECE FOOTER (Made with Streamlit yazısı) estetik için gizlendi, paneller açık */
        ...
        /* LOGIN KUTUSU */
        .login-wrapper { display: flex; justify-content: center; align-items: center; margin-top: 50px; }
        .login-box { background: white; padding: 40px; border-radius: 16px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); border: 1px solid #e2e8f0; width: 100%; max-width: 900px; }

        /* SIDEBAR İÇERİK STİLİ */
        .user-stat { padding: 10px; background: #ffffff; border: 1px solid #e2e8f0; border-radius: 8px; margin-bottom: 8px; font-size: 13px; display: flex; justify-content: space-between; align-items: center; }
...
def dashboard_modu():
    ...
    # --- SIDEBAR (YAN PANEL) - HER ZAMAN GÖRÜNÜR ---
    with st.sidebar:
        ...
        st.metric("Üye / Online", f"{len(users_db)} / {online_count}")
    ...
    if not st.session_state['logged_in']:
        ...
        with col2:
            ...
            with c2:
                ...
                with st.form("reg_f"):
                    r_u = st.text_input("Kullanıcı Adı Seçin")
                    r_p = st.text_input("Şifre Belirleyin", type="password")
