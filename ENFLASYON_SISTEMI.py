...
def dashboard_modu():
    ...
    # --- SIDEBAR ---
    with st.sidebar:
        # 1. CANLI PİYASA KARTLARI (SADE GÖRÜNÜM: İSİM - DEĞER - DEĞİŞİM)
        st.markdown(
        ...
        # HTML Oluşturucu: "Single Quote" bileşeni. 
        # Grafiksizdir. Veriyi asla gizlemez. 
        # Format: Üstte İsim, Ortada Fiyat, Altta Değişim.
        widgets_html = ""
        for sym in symbols:
            widgets_html += f"""
        ...
        # Yükseklik Hesabı: Grafik kalktığı için kartlar daha kısa ama veriler daha okunaklı.
        # Toplam yükseklik = Kart sayısı * (Kart boyu + Boşluk)
        components.html(f'<div style="display:flex; flex-direction:column;">{widgets_html}</div>', height=len(symbols) * 100)
