...
# --- PDF RAPOR MOTORU ---
# --- GELİŞMİŞ PDF RAPOR MOTORU (Web-TÜFE Tasarımı) ---
class PDFReport(FPDF):
    def __init__(self):
        super().__init__()
        self.primary_color = (160, 0, 0) # Web-TÜFE Kırmızısı
        self.secondary_color = (0, 50, 120) # Mavi tonlar
        self.logo_text = "Web-TÜFE"
    
    def header(self):
        if self.page_no() > 1: # Kapak hariç diğer sayfalarda
            self.set_font('Arial', 'B', 10)
            self.set_text_color(*self.primary_color)
            self.cell(0, 10, self.logo_text, 0, 0, 'L')
            self.set_text_color(100, 100, 100)
            self.set_font('Arial', 'I', 8)
            self.cell(0, 10, f'Bulten: {datetime.now().strftime("%B %Y")}', 0, 1, 'R')
    ...
    def create_cover(self, date_str, rate_val):
        ...
        self.cell(0, 15, "ENFLASYON BULTENI", 0, 1, 'C')
        self.set_font('Arial', '', 14)
        self.cell(0, 10, self.clean_text(f"{date_str}"), 0, 1, 'C')
        
        self.ln(40)
        
        # Manşet Rakamı (Büyük Gösterim)
        self.set_font('Arial', 'B', 60)
        self.set_text_color(*self.primary_color)
        self.cell(0, 25, f"%{rate_val}", 0, 1, 'C')
        ...
        self.ln(20)
        self.set_font('Arial', '', 10)
        self.multi_cell(0, 6, self.clean_text("Bu rapor, Web-TUFE sistemi tarafindan otomatik olarak olusturulmus olup, piyasadaki anlik fiyat degisimlerini ve enflasyon trendlerini icermektedir."))
    ...
    def add_plot_image(self, plot_bytes, title="Grafik"):
        if plot_bytes:
            ...
            try:
                ...
                self.cell(0, 10, "Gorsel yuklenemedi.", 0, 1)
            self.ln(5)

    def create_table(self, df):
        # Basit bir tablo oluşturucu (Analiz sekmesindeki tabloyu basar)
        ...
        for col in cols:
            self.cell(col_width, 8, self.clean_text(col), 1, 0, 'C', True)
        self.ln()
        ...
        for _, row in df.iterrows():
            for col in cols:
                ...
                self.cell(col_width, 7, self.clean_text(str(val)), 1, 0, 'C')
            self.ln()
...
def create_pdf_report_advanced(text_content, df_table, figures, manset_oran, date_str):
    ...
    # 2. YÖNETİCİ ÖZETİ (Text Content)
    pdf.add_page()
    pdf.chapter_title("YONETICI OZETI VE STRATEJIK ANALIZ")
    pdf.chapter_body(text_content)
    
    # 3. GRAFİKLER
    # Streamlit figürlerini byte'a çevirip PDF'e gömüyoruz
    if figures:
        pdf.add_page()
        pdf.chapter_title("PIYASA GRAFIKLERI")
        for title, fig in figures.items():
            try:
                # Plotly -> Image dönüşümü
    ...
    if not df_table.empty:
        pdf.add_page()
        pdf.chapter_title("DETAYLI FIYAT HAREKETLERI (TABLO 1)")
        # Sadece önemli sütunları alalım (PDF'e sığması için)
        cols_to_keep = [c for c in df_table.columns if 'Kod' not in c and 'URL' not in c]
        pdf.create_table(df_table[cols_to_keep].head(30)) # İlk 30 satır
...
                # ... (Kodun geri kalanı yukarıda) ...
                ...
                with t_rapor:
                    ...
                    with col_gen:
                        if st.button("✍️ Detaylı Rapor Oluştur", type="primary"):
                            with st.spinner("Stratejik rapor yazılıyor ve görseller işleniyor..."):
                                ...
                                FORMAT: Giriş, Sektörel Analiz, Riskler ve Sonuç. Maddeli ve net olsun.
                                """
                                try:
                    ...
                    if st.session_state['report_text']:
                        ...
                        # --- GRAFİK VE VERİ HAZIRLIĞI ---
                        figures_dict = {}
                        # Önceden oluşturduğun grafikleri buraya ekliyoruz
                        # Not: Bu grafikler (fig_main, fig_hist) dashboard kodunda daha önce create edilmiş olmalı.
                        # Kod akışında t_analiz ve t_istatistik sekmelerinde yaratılan figürleri local variable olarak erişilebilir kılmalısın.
                        # Eğer erişilemiyorsa burada yeniden basitçe oluşturabiliriz:
                        
                        # PDF için basit Trend Grafiği (Garanti olsun diye tekrar çiziyoruz)
                        fig_pdf_trend = px.line(df_trend, x='Tarih', y='TÜFE', title='Enflasyon Trendi')
                        figures_dict["Enflasyon Trendi"] = fig_pdf_trend
                        
                        # PDF için basit Histogram
                        fig_pdf_hist = px.histogram(df_analiz, x="Fark", nbins=20, title="Fiyat Dagilimi")
                        figures_dict["Fiyat Degisim Dagilimi"] = fig_pdf_hist

                        # PDF OLUŞTURMA ÇAĞRISI
                        pdf_bytes = create_pdf_report_advanced(
                            text_content=st.session_state['report_text'],
                            df_table=df_analiz[['Grup', ad_col, 'Fark', son]].sort_values('Fark', ascending=False).head(20), # Örnek tablo
...
BU KODUN PDF RAPORUNU EKTE ATTIĞIM DOSYADAKİ GİBİ OLSUN İSTİYORUM.
